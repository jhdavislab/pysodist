...
def add_args(parser):
    ...
    parser.add_argument('--interp_only', action='store_const', const=True, default=False, help='Only save the interpolated spectra instead of the raw spectra. Optional, default is N15.')
    parser.add_argument('--sum_only', action='store_const', const=True, default=False, help='Only save summed (and interpolated) spectra instead of all individual spectra. Results in 1 spectra per peptide. Optional, default is to extract all spectra.')
    parser.add_argument('--interp_res', default=0.001, type=float, help='Set the interpolation delta m/z - typical values from 0.01 to 0.001. Optional, default is 0.001.')
    return parser
