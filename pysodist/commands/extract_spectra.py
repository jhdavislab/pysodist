...
def extract_sample(sample, config_data, logfile=None):
    ...
    assert os.path.exists(mzml_file) is True, 'Could not find mzML file - check that it is in the mzml_directory, ' \
                                              'has the sample name, and the extension .mzML (note case mzML)'
