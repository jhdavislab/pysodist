...
def extract_spectra(parsed_mzml, parsed_report, output_dir, labeling='N15',
                    interp_res=0.001, sum_spectra_only=False, logfile=None):
    """
...
def mz_window(peptide_modified_sequence, z, mz, labeling='N15', topomers_left=2, topomers_right=5):
    """
    ...
    :param topomers_right: number of isotopomers to include in the extracted spectra to the right of the max mass
    ...
    if labeling == 'N15':
        ...
        labeled_mz = mz
    return [mz - (topomers_left / z), labeled_mz + (topomers_right / z)]


def add_args(parser):
    parser.add_argument('configuration_file', help='full path to configuration file (generated by pysodist configure '
                                                   'or a subsequent command).')
    parser.add_argument('--mzml_directory', default=None,
                        help='Optionally provide a new full mzml directory path. This parameter will override '
                             '(and update) your configuration file. Note that this directory must include one mzML file'
                             'for each sample you will be processing.')
    parser.add_argument('--labeling', default='N15',
                        help='The labeling scheme used for the highest mass isotope envelope you expect to fit. '
                             'Possible values are: N15, C13, K6R6, K8R10. Providing this parameter will override '
                             '(and update) your configuration file')
    parser.add_argument('--logfile', default='pysodist.log',
                        help='Optionally provide a logfile name.')
    return parser


def extract_sample(sample, config_data, logfile=None):
    parsed_report = config_data.loc['output_directory']['VALUE'] + sample + '/pd_parsed_report.tsv'
    assert os.path.exists(parsed_report) is True, 'Could not find pd_parsed_report.tsv. Check that it is in the ' \
                                                  'output_directory, and was generated by pysodist parse_input.py.'

    path_error = '\n\n****ERROR****Could not find provided mzml file: ' + \
                 str(config_data.loc['mzml_directory']['VALUE']) + sample + \
                 '.mzML. Check that this file is present in the mzml_directory and has the sample name and ' \
                 'extension .mzML (note case mzML) or provide the proper path in your config file or at ' \
                 'the command line and try again. \n\n'
    try:
        mzml_file = config_data.loc['mzml_directory']['VALUE'] + sample + '.mzML'
        assert os.path.exists(mzml_file) is True, path_error
    except TypeError:
        print(path_error)
        raise
...
def main(args):
    ...
    if args.mzml_directory is not None:
        log('using mzml_directory provided at command line, will override that provided in the configuration file.')
        config_data.loc['mzml_directory']['VALUE'] = utilities.clean_path(args.mzml_directory)
    ...
    log('writing configuration file. 02_extract.cfg'
        'Please check this file before proceeding to run_isodist.')
    configure.write_config_file(config_data, '02_extract.cfg', logfile)
