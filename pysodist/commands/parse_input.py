...
''' This tool is designed as an initial parser for skyline report files to get them into a common form that pysodist can then use to 
extract spectra. This has been designed to allow for rapid changes to the skyline parser if the report files change, and to allow
other types of 'guides' (anything that tells you retnetion times to exptract for given spectra, e.g. a MS/MS search) to be read and
converted to a fixed file format the pysodist can handle. This common file format requires the following fields for each peptide:
...
def extract_skyline_sub(full_dataframe, sample, protein_list=None, isotope='light'):
    ...
    '''
    required_cols = [isotope+' '+sample+' '+defs.RT_START_FIELD,isotope+' '+sample+' '+defs.RT_END_FIELD,
...
def parse_sub_skyline(skyline_sub_df, sample, q_value=0.00, isotope='light', IO=True):
    ...
    if q_value>0.00:
        filt_entries = skyline_sub_df.loc[skyline_sub_df[isotope+' '+sample+' '+defs.DETECT_Q_VALUE_FIELD] < q_value]
    else:
        filt_entries = skyline_sub_df

    peptides = [rename_peptide(i) for i in filt_entries[defs.PEPTIDE_MOD_SEQ_FIELD]]
    if IO:
        print('found '+str(len(peptides)) + ' peptides.')
    ...
    'charge':zs[i], 'mz':mzs[i], 'protein_IDs':prot_names[i], 'start_pos':pep_start[i], 'end_pos':pep_end[i]} for i in range(used_peptides)}).T
...
def parse_skyline(path_to_skyline_csv, output_directory, sample_list=None, protein_list=None, isotope='light', q_value=0.00, IO=True):
    '''
    Wrapper function to execute extract_skyline_sub and parse_sub_skyline on all proteins of interest. Useful if there are multiple samples in a given report file
    as it will then generate separate pysodist .tsv files for each of these samples.
    ...
    :param sample_list:  list of strings of individual sample identifiers in skyline report (default None results in analysis of all samples)
    :param protein_list: string of protein_preferred_name identifier for particular proteins of interest (default None includes all proteins)
    :param isotope: string for filtering output dataframe such that columns from a particular isotope type are included (default 'light')
    ...
    for sample in sample_list:
        ...
        try:
            ...
            print('...the output directory: ' + write_directory + ' already exists, and files within it may be overwritten. continue? [y/n]')
            choice = input().lower()
            if not choice=='y':
                raise
...
def add_args(parser):
    ...
    parser.add_argument('--protein_list', default=None, help='An optional list of the proteins to parse. By default, all proteins in the report are analyzed.')
    parser.add_argument('--isotope', default='light', help='Be default, it is assumed that the report contains a light isotope (no special labeling), if this field is not present in the report, you can specify a different field here (e.g. "heavy")')
    parser.add_argument('--q_value', default=0.00, help='Used to optionally filter the report file based on the q_value. By default, no q_value filtering is used.')
    return parser
