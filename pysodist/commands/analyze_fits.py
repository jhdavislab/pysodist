...
def parse_isodist_csv(file_path):
    ...
    for CID in all_CIDs:
        all_RTs = np.array(
            [float(i) for i in parsed_id_output[parsed_id_output['CID'] == CID]['retention_time'].values])
        if len(all_RTs) > 0:
            ...
            parsed_id_output.loc[parsed_id_output['CID'] == CID, 'clean_RT'] = \
                parsed_id_output[parsed_id_output['CID'] == CID]['retention_time']  # do we need this
            parsed_id_output.loc[parsed_id_output['CID'] == CID, 'peak_position'] = \
                (parsed_id_output[parsed_id_output['CID'] == CID]['clean_RT'].astype(
                    float) - min_RT) / range_RT  # do we need this
...
def add_args(parser):
    parser.add_argument('isodist_result', type=str, required=True, help='Isodist configuration output file (.cfg)')
    parser.add_argument('primary_channel', type=str, default='AMP_F',
                        help='Isotope channel to fit and filter by (AMP_U, AMP_L, AMP_F, or sum). Default is AMP_F')
    parser.add_argument('output_directory', type=str, help='Full path to directory for storing output plots and files')
    parser.add_argument('--threads', type=int, default=2, help='Number of threads to use in parallel')
    parser.add_argument('--plot', type=bool, default=True, help='Whether to plot retained and filtered-out spectra')
    parser.add_argument('--expect', nargs='+', default=["U/F"], type=list,
                        help='Expected ["U/F", "L/F", "U/L"] ratios')  # not sure what this does
...
if __name__ == "__main__":
    ...
    log('\n****INITIATING ANALYZE_FITS****', args.logfile)
    log('executed command: ' + " ".join(sys.argv), args.logfile)
    log('loading data...',  args.logfile)
    wd = args.workdir
    ...
    for pep in all_peps:
        ...
        sub_pep = pep_df[
            (pep_df['AMP_U'] >= 0.1 * pep_meds['AMP_U']) & (pep_df['AMP_L'] >= 0.1 * pep_meds['AMP_L']) & (
    ...
    fit_results, failed_fits = plot_fits_spectra(gw_amp_filt, args.filt, gaussian, fit_results, result_cols,
                                                storefits,
                                                ft, args.threads, wd)
    ...
    f_filt = fit_results[fit_results['SSR_gaussian_' + abbrev] <=
                         np.percentile(fit_results['SSR_gaussian_' + abbrev],args.ssr)].index

    expected_ratios = args.expect
    subf = gw_amp_filt.loc[f_filt, ratio_cols]

    log('plotting gaussian amplitude filter boxplots', args.logfile)
    fig, ax = plt.subplots(1, 1)
    nonsum_x = [0, 1.5, 3]
